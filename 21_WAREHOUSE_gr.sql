...
if vvcheckpro > 0 then
  ...
  if (vvfund - :new.ORDI_QUANTITY) >= 0 then
       ...
       CLOSE DISPLAY_PRODUCT;
        ...
        p_message          => 'Δεν επαρκεί το απόθεμα του προϊόντος <b>' ||vvproname|| '</b> στην αποθήκη <b>' ||vvwarname|| '</b> <br>' ||
                               'Διαθέσιμο απόθεμα <b>' ||vvfund|| '</b> τεμάχια',
        p_display_location => apex_error.c_inline_in_notification );  
  end if;
else
    
apex_error.add_error (
p_message          => 'Δεν υπάρχει το προϊόν στην αποθήκη που επιλέξατε!!!',
p_display_location => apex_error.c_inline_in_notification );     
end if;
